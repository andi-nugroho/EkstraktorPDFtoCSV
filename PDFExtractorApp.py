...
class PDFExtractorApp:
    def __init__(self, root):
        ...
        # Ubah ikon aplikasi (Dukung icon.png sebagai alternatif icon.ico)
        try:
            if os.path.exists("icon.ico"):
                ...
                icon_img = PIL.Image.open("icon.png")
                icon_img = ImageTk.PhotoImage(icon_img)
                self.root.tk.call('wm', 'iconphoto', self.root._w, icon_img)
        except Exception as e:
            console.print(f"[bold red]⚠️ Gagal memuat ikon: {e}[/bold red]")
    ...
    def ekstrak_pdf(self, pdf_path):
        ...
        try:
            ...
            if format_terpilih in ["csv", "both"]:
                df.to_csv(os.path.join(output_dir, f"{pdf_name}.csv"), index=False)
            if format_terpilih in ["xlsx", "both"]:
                df.to_excel(os.path.join(output_dir, f"{pdf_name}.xlsx"), index=False)

            self.tampilkan_hasil_terminal(df)
            self.preview_data(df)
            winsound.Beep(1000, 300)  # Notifikasi suara sukses
            messagebox.showinfo("Sukses", "✅ Ekstraksi berhasil!")
        except Exception as e:
            winsound.Beep(500, 300)  # Notifikasi suara error
    ...
    def preview_data(self, df):
        for widget in self.table_frame.winfo_children():
            widget.destroy()
        PandasTable(self.table_frame, dataframe=df, showtoolbar=True, showstatusbar=True).show()

    def ganti_tema(self):
        self.style.theme_use("darkly" if self.style.theme == "superhero" else "superhero")
