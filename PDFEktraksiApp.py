...
class PDFExtractorApp:
    def __init__(self, root):
        ...
        # Tambahkan ikon aplikasi (dukung PNG & ICO)
        if os.path.exists("icon.ico"):
            self.root.iconbitmap("icon.ico")
        elif os.path.exists("icon.png"):
            self.set_png_icon("icon.png")
    ...
    def ekstrak_pdf(self, pdf_path):
        self.log_message(f"🔄 Memulai ekstraksi data dari: {os.path.basename(pdf_path)}...")
        ...
        try:
            ...
            messagebox.showinfo("Sukses", f"✅ Ekstraksi berhasil untuk: {pdf_name}")
    ...
    def play_sound(self, sound_type):
        if platform.system() == "Windows":
            threading.Thread(target=lambda: winsound.MessageBeep(winsound.MB_OK if sound_type == "success" else winsound.MB_ICONHAND)).start()
